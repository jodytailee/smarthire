...
import { generarTextoRechazo } from "@/lib/postulantes-ia";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: post, error: fetchErr } = await db
    ...
    .select("primer_nombre, primer_apellido, correo_electronico, respuestas, puesto, texto_rechazo, estado")
    .eq("id", id)
  ...
  if (action === "generar") {
    ...
    const textoGenerado = await generarTextoRechazo({
      empresa,
    ...
    await db.from("vacante_postulacion").update({ texto_rechazo: textoGenerado }).eq("id", id).eq("fk_empresa", usuario.fkEmpresa);
    return NextResponse.json({ ok: true, texto: textoGenerado });
  }

  if (action === "enviar") {
    const raw = texto?.trim() || post.texto_rechazo;
    if (!raw) return NextResponse.json({ error: "No hay texto de rechazo para enviar." }, { status: 400 });
    ...
    const { error: updateErr } = await db
      .from("vacante_postulacion")
      .update({ estado: "rechazado", texto_rechazo: textoFinal, rechazo_enviado_at: new Date().toISOString() })
      .eq("id", id)
      .eq("fk_empresa", usuario.fkEmpresa);
